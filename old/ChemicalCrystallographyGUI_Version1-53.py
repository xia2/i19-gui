                    ...
                    if prefix == "":
                        ...
                        if runSelection:
                            for entry in runSelection:
                                xia2Input=xia2Input+" image="+datasetPath+prefix+str("_%02d_00001.cbf" % int(entry))
                                if entry in imageSelection:
                                    xia2Input=xia2Input+":"+imageSelection[entry]
                        else:                          # runs have NOT been selected
                            for run in runs:
                                xia2Input=xia2Input+" image="+datasetPath+prefix+str("_%02d_00001.cbf" % int(run))
    ...
    def runXia2dataset(self,inputDataset, xia2Input): # prefix # visit # dataset
        ...
        ###################################################################################################################
            ...
            ###################################################################################################################
                ...
                self.thread=mythread2(self.processingPath,inputDataset,self.tabstxt,self.tabsNum,self.mainTab_txt)
            ...
            if runImageSelector == False:
                ...
                if self.datasetPath == "":
                    ...
                    if runSelection:            # runs have been selected
                        for entry in runSelection:
                            datasetInput=datasetInput+" image="+self.datasetPath+self.prefix+str("_%02d_00001.cbf" % int(entry))
                            if (entry-1) in imageSelection:
                                datasetInput=datasetInput+":"+imageSelection[entry-1]
                    else:                          # runs have NOT been selected
                        for run in self.runList:
                            datasetInput=datasetInput+" image="+self.datasetPath+self.prefix+str("_%02d_00001.cbf" % int(run))
                            if (run-1) in imageSelection:
