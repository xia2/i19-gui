...
### functions ######
    ...
    def selectDataset(self):
        try:
            ...
            self.command_command.setPlainText(self.xia2command + datasetINPUT + xia2OptionsList)
        except:
    ...
    def openMultiple(self):
        #try:
            path=self.openingVisit
            newDatasetPath = QFileDialog.getExistingDirectory(None, 'Select a dataset folder', path,QFileDialog.ShowDirsOnly)
            ...
            runList.sort()
            self.appendOutput(self.mainTab_txt,"	New Number of runs:		"+str(len(runList))+" "+ str(runList))  
            for run in runList:				# number of images per run
                ...
                runImagesDict[run]=numCbfRun
            self.appendOutput(self.mainTab_txt,"	New Images per run: 	"+str(runImagesDict))
            totalNumImages=sum(runImagesDict.values())		# total number of imagess
            self.appendOutput(self.mainTab_txt,"	New Total number of images:	"+str(totalNumImages)+"\n")
            # update lables
            ...
            self.appendOutput(self.mainTab_txt,"	Multiple runs:\n	"+str(self.multipleDataset))
    ...
    def openOptions(self):
        ...
        self.ui = Ui_Xia2Options(self.xia2command, self.datasetPath, self.command_command, self.visit, self.mainTab_txt, self.runList, self.prefix, self.openingVisit)
        ...
        if datasetINPUT == "MULTIPLE":       # if multiple input has been utilised
            ...
            if runImageSelector == False:
                for datasetKey in self.multipleDataset:
                    ...
                    runList = self.multipleDataset[datasetKey][2]
                    if prefix == "":
                        self.appendOutput(self.mainTab_txt,"\n\n ########################################################b")
                        self.appendOutput(self.mainTab_txt,"	No cbf images found in directory, please select dataset directory")
                        return
                    ...
                    if prefix == "":
                        ...
                        if runSelection:
                            for entry in runSelection:
                                xia2Input=xia2Input+" image="+datasetPath+prefix+str("%02d_00001.cbf" % int(entry))
                                if entry in imageSelection:
                                    xia2Input=xia2Input+":"+imageSelection[entry]
                        else:                          # runs have NOT been selected
                            for run in runs:
                                xia2Input=xia2Input+" image="+datasetPath+prefix+str("%02d_00001.cbf" % int(run))
                                if (run-1) in imageSelection:
                                    xia2Input=xia2Input+":"+imageSelection[run-1]
    ...
    def runXia2dataset(self,inputDataset, xia2Input): # prefix # visit # dataset
            ...
            self.processingPath=self.visit + "processing/xia2GUI/" + inputDataset + "_" + timeDate + "/"
            ...
            inputXia2Command=self.xia2command + xia2Input + xia2OptionsList   # this is the bit I think i need to change!!! # dataset and prefix I would guess is required
	    ...
	    ### open new tab with dataset and date
            ...
            self.xia2output.setTabText(self.xia2output.indexOf(self.tabs[self.tabsNum]), inputDataset + "_" + timeDate)
            ...
            if inputDataset == "":
                ...
                if computingLocation == "Local":
                    ...
                    if "staging" in self.visit.split("/"):
                        os.system("module load global/cluster && qsub -pe smp 1 -q medium.q " + jobFile)
                    else:
                        os.system("module load global/hamilton && qsub -pe smp 20 -q all.q -P i19-2 -o /dev/null -e /dev/null " + jobFile)
   
            ###################################################################################################################
                #output xia2.txt into tab
                self.thread=mythread2(self.processingPath,inputDataset,self.tabstxt,self.tabsNum,self.mainTab_txt,"xia2.txt")
    ...
    def runScreen19(self):
        ...
        if self.prefix == "":
            ...
            # create processing path
            timeDate=str(datetime.utcnow().strftime("%Y%m%d_%H%M"))
            self.processingPath=self.visit + "processing/xia2GUI/" + self.dataset + "_s19_" + timeDate + "/"
            if not os.path.exists(self.visit + "processing/xia2GUI/"):
                os.makedirs(self.visit + "processing/xia2GUI/")
            if not os.path.exists(self.processingPath):
                os.makedirs(self.processingPath)
	    ...
	    ### open new tab with dataset and date
            ...
            self.xia2output.addTab(self.tabs[self.tabsNum], "")
            self.xia2output.setTabText(self.xia2output.indexOf(self.tabs[self.tabsNum]), self.dataset + "_s19_" + timeDate)

            #edit plain text
            self.tabstxt[self.tabsNum].appendPlainText("\nRunning screen19\n")
            self.tabstxt[self.tabsNum].appendPlainText("screen19 command:")
            self.tabstxt[self.tabsNum].appendPlainText("	" + inputScreen19Command + "\n")
        ...
        ###################################################################################################################
            ...
            ###################################################################################################################
                #output xia2.txt into tab
                self.thread=mythread2(self.processingPath,self.dataset,self.tabstxt,self.tabsNum,self.mainTab_txt,"screen19.log")
...
####################################################
	...
	# Version menu
        ...
        self.viewButtons_screen19.setStatusTip(_translate("MainWindow", "Run screen with current dataset and options"))  
...
class Ui_Xia2Options():
    def __init__(self, xia2command, datasetPath, command_command, visit, mainTab_txt, runList, prefix, openingVisit): 
    ...
    def setupUi(self, Xia2Options):
        ...
        self.Import_FixBeamDetector_checkBox = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_FixBeamDetector_checkBox.setGeometry(QtCore.QRect(10, 170, 151, 20))
        self.Import_FixBeamDetector_checkBox.setObjectName("Import_FixBeamDetector_checkBox")
        ...
        self.Import_RunSelector_checkBox_1 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_1.setGeometry(QtCore.QRect(110, 200, 31, 20))
        self.Import_RunSelector_checkBox_1.setObjectName("Import_RunSelector_checkBox_1")
        self.Import_RunSelector_checkBox_2 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_2.setGeometry(QtCore.QRect(150, 200, 31, 20))
        self.Import_RunSelector_checkBox_2.setObjectName("Import_RunSelector_checkBox_2")
        self.Import_RunSelector_checkBox_3 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_3.setGeometry(QtCore.QRect(190, 200, 31, 20))
        self.Import_RunSelector_checkBox_3.setObjectName("Import_RunSelector_checkBox_3")
        self.Import_RunSelector_checkBox_4 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_4.setGeometry(QtCore.QRect(230, 200, 31, 20))
        self.Import_RunSelector_checkBox_4.setObjectName("Import_RunSelector_checkBox_4")
        self.Import_RunSelector_checkBox_5 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_5.setGeometry(QtCore.QRect(270, 200, 31, 20))
        self.Import_RunSelector_checkBox_5.setObjectName("Import_RunSelector_checkBox_5")
        self.Import_RunSelector_checkBox_6 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_6.setGeometry(QtCore.QRect(310, 200, 31, 20))
        self.Import_RunSelector_checkBox_6.setObjectName("Import_RunSelector_checkBox_6")
        self.Import_RunSelector_checkBox_7 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_7.setGeometry(QtCore.QRect(350, 200, 31, 20))
        self.Import_RunSelector_checkBox_7.setObjectName("Import_RunSelector_checkBox_7")
        self.Import_RunSelector_checkBox_8 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_8.setGeometry(QtCore.QRect(390, 200, 31, 20))
        self.Import_RunSelector_checkBox_8.setObjectName("Import_RunSelector_checkBox_8")
        self.Import_RunSelector_checkBox_9 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_9.setGeometry(QtCore.QRect(430, 200, 31, 20))
        self.Import_RunSelector_checkBox_9.setObjectName("Import_RunSelector_checkBox_9")
        self.Import_RunSelector_checkBox_10 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_10.setGeometry(QtCore.QRect(470, 200, 41, 20))
        self.Import_RunSelector_checkBox_10.setObjectName("Import_RunSelector_checkBox_10")
        self.Import_RunSelector_checkBox_11 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_11.setGeometry(QtCore.QRect(510, 200, 41, 20))
        self.Import_RunSelector_checkBox_11.setObjectName("Import_RunSelector_checkBox_11")
        self.Import_RunSelector_checkBox_12 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_12.setGeometry(QtCore.QRect(550, 200, 41, 20))
        self.Import_RunSelector_checkBox_12.setObjectName("Import_RunSelector_checkBox_12")

        self.Import_RunSelector_checkBox_13 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_13.setGeometry(QtCore.QRect(110, 230, 41, 20))
        self.Import_RunSelector_checkBox_13.setObjectName("Import_RunSelector_checkBox_13")
        self.Import_RunSelector_checkBox_14 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_14.setGeometry(QtCore.QRect(150, 230, 41, 20))
        self.Import_RunSelector_checkBox_14.setObjectName("Import_RunSelector_checkBox_14")
        self.Import_RunSelector_checkBox_15 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_15.setGeometry(QtCore.QRect(190, 230, 41, 20))
        self.Import_RunSelector_checkBox_15.setObjectName("Import_RunSelector_checkBox_15")
        self.Import_RunSelector_checkBox_16 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_16.setGeometry(QtCore.QRect(230, 230, 41, 20))
        self.Import_RunSelector_checkBox_16.setObjectName("Import_RunSelector_checkBox_16")
        self.Import_RunSelector_checkBox_17 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_17.setGeometry(QtCore.QRect(270, 230, 41, 20))
        self.Import_RunSelector_checkBox_17.setObjectName("Import_RunSelector_checkBox_17")
        self.Import_RunSelector_checkBox_18 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_18.setGeometry(QtCore.QRect(310, 230, 41, 20))
        self.Import_RunSelector_checkBox_18.setObjectName("Import_RunSelector_checkBox_18")
        self.Import_RunSelector_checkBox_19 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_19.setGeometry(QtCore.QRect(350, 230, 41, 20))
        self.Import_RunSelector_checkBox_19.setObjectName("Import_RunSelector_checkBox_19")
        self.Import_RunSelector_checkBox_20 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_20.setGeometry(QtCore.QRect(390, 230, 41, 20))
        self.Import_RunSelector_checkBox_20.setObjectName("Import_RunSelector_checkBox_20")
        self.Import_RunSelector_checkBox_21 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_21.setGeometry(QtCore.QRect(430, 230, 41, 20))
        self.Import_RunSelector_checkBox_21.setObjectName("Import_RunSelector_checkBox_21")
        self.Import_RunSelector_checkBox_22 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_22.setGeometry(QtCore.QRect(470, 230, 41, 20))
        self.Import_RunSelector_checkBox_22.setObjectName("Import_RunSelector_checkBox_22")
        self.Import_RunSelector_checkBox_23 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_23.setGeometry(QtCore.QRect(510, 230, 41, 20))
        self.Import_RunSelector_checkBox_23.setObjectName("Import_RunSelector_checkBox_23")
        self.Import_RunSelector_checkBox_24 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_24.setGeometry(QtCore.QRect(550, 230, 41, 20))
        self.Import_RunSelector_checkBox_24.setObjectName("Import_RunSelector_checkBox_24")

        self.Import_RunSelector_checkBox_25 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_25.setGeometry(QtCore.QRect(110, 260, 41, 20))
        self.Import_RunSelector_checkBox_25.setObjectName("Import_RunSelector_checkBox_25")
        self.Import_RunSelector_checkBox_26 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_26.setGeometry(QtCore.QRect(150, 260, 41, 20))
        self.Import_RunSelector_checkBox_26.setObjectName("Import_RunSelector_checkBox_26")
        self.Import_RunSelector_checkBox_27 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_27.setGeometry(QtCore.QRect(190, 260, 41, 20))
        self.Import_RunSelector_checkBox_27.setObjectName("Import_RunSelector_checkBox_27")
        self.Import_RunSelector_checkBox_28 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_28.setGeometry(QtCore.QRect(230, 260, 41, 20))
        self.Import_RunSelector_checkBox_28.setObjectName("Import_RunSelector_checkBox_28")
        self.Import_RunSelector_checkBox_29 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_29.setGeometry(QtCore.QRect(270, 260, 41, 20))
        self.Import_RunSelector_checkBox_29.setObjectName("Import_RunSelector_checkBox_29")
        self.Import_RunSelector_checkBox_30 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_30.setGeometry(QtCore.QRect(310, 260, 41, 20))
        self.Import_RunSelector_checkBox_30.setObjectName("Import_RunSelector_checkBox_30")
        self.Import_RunSelector_checkBox_31 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_31.setGeometry(QtCore.QRect(350, 260, 41, 20))
        self.Import_RunSelector_checkBox_31.setObjectName("Import_RunSelector_checkBox_31")
        self.Import_RunSelector_checkBox_32 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_32.setGeometry(QtCore.QRect(390, 260, 41, 20))
        self.Import_RunSelector_checkBox_32.setObjectName("Import_RunSelector_checkBox_32")
        self.Import_RunSelector_checkBox_33 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_33.setGeometry(QtCore.QRect(430, 260, 41, 20))
        self.Import_RunSelector_checkBox_33.setObjectName("Import_RunSelector_checkBox_33")
        self.Import_RunSelector_checkBox_34 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_34.setGeometry(QtCore.QRect(470, 260, 41, 20))
        self.Import_RunSelector_checkBox_34.setObjectName("Import_RunSelector_checkBox_34")
        self.Import_RunSelector_checkBox_35 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_35.setGeometry(QtCore.QRect(510, 260, 41, 20))
        self.Import_RunSelector_checkBox_35.setObjectName("Import_RunSelector_checkBox_35")
        self.Import_RunSelector_checkBox_36 = QtWidgets.QCheckBox(self.xia2options_Import)
        self.Import_RunSelector_checkBox_36.setGeometry(QtCore.QRect(550, 260, 41, 20))
        self.Import_RunSelector_checkBox_36.setObjectName("Import_RunSelector_checkBox_36")
...
#Index_maxCell_lineEdit
        ...
        self.Index_multiSweepRefine_checkBox = QtWidgets.QCheckBox(self.xia2options_Indexing)
        self.Index_multiSweepRefine_checkBox.setGeometry(QtCore.QRect(10, 190, 261, 20))
        self.Index_multiSweepRefine_checkBox.setObjectName("Index_multiSweepRefine_checkBox")
        ...
        self.Other_clusterOrLocal_comboBox.setObjectName("Other_clusterOrLocal_comboBox")
        ...
        self.HP_correction_shaddowing_checkBox = QtWidgets.QCheckBox(self.xia2options_HP)
        self.HP_correction_shaddowing_checkBox.setGeometry(QtCore.QRect(10, 20, 321, 20))
        self.HP_correction_shaddowing_checkBox.setObjectName("HP_correction_shaddowing_checkBox")
        ...
        self.HP_ReferenceGeometry_checkBox.setObjectName("HP_ReferenceGeometry_checkBox")
        ...
        self.HP_anvilOpeningAngle_checkBox.setObjectName("HP_anvilOpeningAngle_checkBox")
        ...
        self.HP_anvilOpeningAngle_lineEdit.setObjectName("HP_anvilOpeningAngle_lineEdit")
        ...
        self.runSelectorList=[self.Import_RunSelector_checkBox_1,
        ...
        self.optionListRefineScale=[self.Refine_FixBeamDetector_checkBox,
        ...
        self.optionListHP=[self.HP_correction_shaddowing_checkBox,
        ...
        self.runStartEnd_lineEdits=[self.HP_runStartEnd_lineEdit_1.text(),
    ...
    def updateOptions(self):
        ...
        for varible in self.optionListImport:
            if varible.isChecked():
                ...
                if varible == self.Import_FixBeamDetector_checkBox:
                    options = options + " integrate.phil_file=/dls_sw/i19/scripts/HP/intergration_additional_inputs.phil"
                ...
                if varible == self.Import_RunSelector_checkBox:
                    ...
                    self.mainTab_txt.appendPlainText("Run selector:	"+str(runSelection))
                    self.mainTab_txt.moveCursor(QtGui.QTextCursor.End)
        ...
        for varible in self.optionListHP:
            if varible.isChecked():
                if varible == self.HP_correction_shaddowing_checkBox:
                    options = options + " high_pressure.correction=True dynamic_shadowing=True resolution_range=999,15"
                ...
                if varible == self.HP_gasket_checkBox:
                    ...
                    if gasketType == "Tungsten":
                        options = options + " ice_rings.filter=True ice_rings.unit_cell=3.1652,3.1652,3.1652,90,90,90 ice_rings.space_group=Im-3m ice_rings.width=0.02"
                    if gasketType == "Steel":
                        # steel gaskets are often either Fe or Ni. The unit cell for Fe is given below. 
                        options = options + " ice_rings.filter=True ice_rings.unit_cell=2.87,2.87,2.87,90,90,90 ice_rings.space_group=Im-3m ice_rings.width=0.02"

                if varible == self.HP_gasketUser_checkBox:
                    powderRing_lineEdits=[self.HP_gasketUserUC_lineEdit.text(),
                    ...
                    for entry in powderRing_lineEdits:
                        ...
                        ice_rings_UV_command = " ice_rings.unit_cell=" + str(self.HP_gasketUserUC_lineEdit.text())
                        ice_rings_SG_command = " ice_rings.space_group=" + str(self.HP_gasketUserSG_lineEdit.text())
                        ice_rings_W_command = " ice_rings.width=" + str(self.HP_gasketUserW_lineEdit.text())
                        options = options + ice_rings_UV_command + ice_rings_SG_command + ice_rings_W_command
                # medium difficulty:
                if varible == self.HP_UN_SG_checkBox:
                    UC_SG_lineEdits=[self.HP_UN_lineEdit.text(),
                ...
                if varible == self.HP_dmin_checkBox:
                    if self.HP_dmin_lineEdit.text() == "":
                        outputMessage="	*** D_min HP Error, please entre d_min e.g. 0.84 ***"
                        self.mainTab_txt.appendPlainText(outputMessage)
                        self.mainTab_txt.moveCursor(QtGui.QTextCursor.End)
                        return
                    else:                   
                        options = options + " d_min=" + str(self.HP_dmin_lineEdit.text())
                ...
                if varible == self.HP_anvilThickness_checkBox:
                    if self.HP_anvilThickness_lineEdit.text() == "":
                        ...
                        options = options + " high_pressure.anvil.thickness=" + str(self.HP_anvilThickness_lineEdit.text())
        ...
        if not datasetINPUT == "MULTIPLE":
            ...
            if runImageSelector == False:
                ...
                if self.datasetPath == "":
                    datasetInput=self.datasetPath
                    self.mainTab_txt.appendPlainText("Dataset must be selected before selecting runs or images start/end")
                    self.mainTab_txt.moveCursor(QtGui.QTextCursor.End)
                else:
                    datasetInput=""
                    if runSelection:            # runs have been selected
                        for entry in runSelection:
                            datasetInput=datasetInput+" image="+self.datasetPath+"/"+self.prefix+str("%02d_00001.cbf" % int(entry))
                            if (entry-1) in imageSelection:
                                datasetInput=datasetInput+":"+imageSelection[entry-1]
                    else:                          # runs have NOT been selected
                        for run in self.runList:
                            datasetInput=datasetInput+" image="+self.datasetPath+"/"+self.prefix+str("%02d_00001.cbf" % int(run))
                            if (run-1) in imageSelection:
                                datasetInput=datasetInput+":"+imageSelection[run-1]
        ...
        Ui_MainWindow.updateOptionsFunction(self.xia2command,datasetINPUT,self.command_command)
    ...
    def resetOptions(self):
        ...
        if os.path.isfile(self.visit + "processing/autoSaveOptions.txt"):
            ...
            with open(optionFile, "w") as oF: pass
    ...
    def optionFileTextFunction(self):
        ...
        for num, checkboxes in enumerate(self.optionListImport):
            if checkboxes.isChecked():
                if num == 1: 
                    optionFileText = optionFileText + "I " + str(num) + " " + str(self.refGeometryPath) + "\n"
                elif num == 2: 
                    optionFileText = optionFileText + "I " + str(num) + " " + str(self.import_DD_lineEdit.text()) + "\n"
                elif num == 3: 
                    optionFileText = optionFileText + "I " + str(num) + " " + str(self.import_BeamCentre_X_lineEdit.text()) + " " + str(self.import_BeamCentre_Y_lineEdit.text())+ "\n"
                elif num == 4: 
                    optionFileText = optionFileText + "I " + str(num) + " " + str(self.import_wavelength_lineEdit.text()) + "\n"
                elif num == 7: 
                    optionFileText = optionFileText + "I " + str(num) + " " + str(self.Import_type_comboBox.currentIndex()) + "\n"
                else:
        ...
        for num, checkboxes in enumerate(self.optionListSpotFinding):
            if checkboxes.isChecked():
                if num == 0: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_sigmaStrong_lineEdit.text()) + "\n"
                elif num == 1: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_minSpot_lineEdit.text()) + "\n"
                elif num == 2: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_maxSpot_lineEdit.text()) + "\n"
                elif num == 3: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_dmin_lineEdit.text()) + "\n"
                elif num == 4: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_dmax_lineEdit.text()) + "\n"
                elif num == 6: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_powderRingsUC_lineEdit.text()) + " " + str(self.findSpots_powderRingsSG_lineEdit.text()) + " " + str(self.findSpots_powderRingsW_lineEdit.text()) + "\n"
                elif num == 7: 
                    ...
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_circleMask_lineEdit.text()) + "\n"
                elif num == 9: 
                    optionFileText = optionFileText + "SF " + str(num) + " " + str(self.findSpots_recMask_lineEdit.text()) + "\n"
                else:
                    optionFileText = optionFileText + "SF " + str(num) + "\n"
        #indexing 
        for num, checkboxes in enumerate(self.optionListIndexing):
            if checkboxes.isChecked():
                if num == 0: 
                    optionFileText = optionFileText + "Ind " + str(num) + " " + str(self.Index_method_comboBox.currentIndex()) + "\n"
                elif num == 2: 
                    optionFileText = optionFileText + "Ind " + str(num) + " " + str(self.Index_UN_lineEdit.text()) + " " + str(self.Index_SG_lineEdit.text()) + "\n"
                elif num == 3: 
                    optionFileText = optionFileText + "Ind " + str(num) + " " + str(self.Index_minCell_lineEdit.text()) + "\n"
                elif num == 4: 
                    optionFileText = optionFileText + "Ind " + str(num) + " " + str(self.Index_maxCell_lineEdit.text()) + "\n"
                else:
        ...
        for num, checkboxes in enumerate(self.optionListIntegrate):
            if checkboxes.isChecked():
                if num == 2: 
                    optionFileText = optionFileText + "Int " + str(num) + " " + str(self.Integrate_minCellOverall_lineEdit.text()) + " " + str(self.Integrate_minCellDegree_lineEdit.text()) + "\n"
                else:
        ...
        for num, checkboxes in enumerate(self.optionListRefineScale):
            if checkboxes.isChecked():
                if num == 1: 
                    optionFileText = optionFileText + "R " + str(num) + " " + str(self.Refine_method_comboBox.currentIndex()) + "\n"
                else:
        ...
        for num, checkboxes in enumerate(self.optionListOther):
            if checkboxes.isChecked():
                if num == 1:
                    optionFileText = optionFileText + "O " + str(num) + " " + str(self.Other_manualInput1_lineEdit.text()) + "\n"
                if num == 2:
                    optionFileText = optionFileText + "O " + str(num) + " " + str(self.Other_manualInput1_lineEdit.text()) + "\n"
                if num == 3:
                    optionFileText = optionFileText + "O " + str(num) + " " + str(self.Other_manualInput1_lineEdit.text()) + "\n"
        ...
        for num, checkboxes in enumerate(self.optionListHP):
            if checkboxes.isChecked():
                if num == 1: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.refGeometryPath) + "\n"
                elif num == 2: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_gasket_comboBox.currentIndex()) + "\n"
                elif num == 3: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_gasketUserUC_lineEdit.text()) + " " + str(self.HP_gasketUserSG_lineEdit.text()) + " " + str(self.HP_gasketUserW_lineEdit.text()) + "\n"
                elif num == 4: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_UN_lineEdit.text()) + " " + str(self.HP_SG_lineEdit.text()) + "\n"
                elif num == 5: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_dmin_lineEdit.text()) + "\n"
                elif num == 6: 
                    ...
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_anvilThickness_lineEdit.text()) + "\n"
                elif num == 9: 
                    optionFileText = optionFileText + "HP " + str(num) + " " + str(self.HP_anvilOpeningAngle_checkBox.text()) + "\n"
    ...
    def loadOptions(self):
        ...
        if self.savedOptionsPath:
            ...
            savedOptionsFileTxt= self.savedOptionsPath.split("/")[-1]
        self.loadOptionsMain(savedOptionsPathTxt)
    ...
    def loadOptionsMain(self,savedOptionsPathTxt):
        ...
        try:
                ...
                for line in optionsInput:
                    ...
                    if lineSplit[0] == "SF":
                        if int(lineSplit[1]) == 0:
                            ...
                            self.optionListSpotFinding[int(lineSplit[1])].setChecked(True)
                        if int(lineSplit[1]) == 1:
                        ...
                        if int(lineSplit[1]) == 6:
                            ...
                            self.optionListSpotFinding[int(lineSplit[1])].setChecked(True)
                        if int(lineSplit[1]) == 7:
                            self.findSpots_resolutionRange_lineEdit_1.setText(lineSplit[2])
                            self.findSpots_resolutionRange_lineEdit_2.setText(lineSplit[3])
                            self.findSpots_resolutionRange_lineEdit_3.setText(lineSplit[4])
                            self.findSpots_resolutionRange_lineEdit_4.setText(lineSplit[5])
                            self.findSpots_resolutionRange_lineEdit_5.setText(lineSplit[6])
                            self.findSpots_resolutionRange_lineEdit_6.setText(lineSplit[7])
                            self.findSpots_resolutionRange_lineEdit_7.setText(lineSplit[8])
                            self.findSpots_resolutionRange_lineEdit_8.setText(lineSplit[9])
                            self.findSpots_resolutionRange_lineEdit_9.setText(lineSplit[10])
                            self.findSpots_resolutionRange_lineEdit_10.setText(lineSplit[11])
                            self.optionListSpotFinding[int(lineSplit[1])].setChecked(True)
                    ...
                    if lineSplit[0] == "Ind":
                        if int(lineSplit[1]) == 0:
                            self.Index_method_comboBox.setCurrentIndex(int(lineSplit[2]))
                    ...
                    if lineSplit[0] == "Int":
                        if int(lineSplit[1]) == 2:
                            self.Integrate_minCellOverall_lineEdit.setText((lineSplit[2]))
                            self.Integrate_minCellDegree_lineEdit.setText((lineSplit[3]))
                            self.optionListIntegrate[int(lineSplit[1])].setChecked(True)
                        else:
                            self.optionListIntegrate[int(lineSplit[1])].setChecked(True) 
                    if lineSplit[0] == "R":
                        if int(lineSplit[1]) == 1:
                            self.Refine_method_comboBox.setCurrentIndex(int(lineSplit[2]))
                            self.optionListRefineScale[int(lineSplit[1])].setChecked(True)
                        else:
                            self.optionListRefineScale[int(lineSplit[1])].setChecked(True) 
                    if lineSplit[0] == "O":
                        if int(lineSplit[1]) == 1:
                    ...
                    if lineSplit[0] == "HP":
                        ...
                        if int(lineSplit[1]) == 6:
                            #### if # then skip ###
                            if not "#" in lineSplit[2]: self.HP_runStartEnd_lineEdit_1.setText((lineSplit[2]))
                            if not "#" in lineSplit[3]: self.HP_runStartEnd_lineEdit_2.setText((lineSplit[3]))
                            if not "#" in lineSplit[4]: self.HP_runStartEnd_lineEdit_3.setText((lineSplit[4]))
                            if not "#" in lineSplit[5]: self.HP_runStartEnd_lineEdit_4.setText((lineSplit[5]))
                            if not "#" in lineSplit[6]: self.HP_runStartEnd_lineEdit_5.setText((lineSplit[6]))
                            if not "#" in lineSplit[7]: self.HP_runStartEnd_lineEdit_6.setText((lineSplit[7]))
                            if not "#" in lineSplit[8]: self.HP_runStartEnd_lineEdit_7.setText((lineSplit[8]))
                            if not "#" in lineSplit[9]: self.HP_runStartEnd_lineEdit_8.setText((lineSplit[9]))
                            if not "#" in lineSplit[10]: self.HP_runStartEnd_lineEdit_9.setText((lineSplit[10]))
                            if not "#" in lineSplit[11]: self.HP_runStartEnd_lineEdit_10.setText((lineSplit[11]))
                            self.optionListHP[int(lineSplit[1])].setChecked(True)
                        if int(lineSplit[1]) == 8:
            ...
            outputMessage="	Error in loading saved options ("+savedOptionsPathTxt+")"
            self.mainTab_txt.appendPlainText(outputMessage)
            self.mainTab_txt.moveCursor(QtGui.QTextCursor.End)
    ...
    def retranslateUi(self, Xia2Options):
        ...
        self.saveButton.setStatusTip(_translate("Xia2Options", "Save the current options"))
        self.loadButton.setText(_translate("Xia2Options", "Load"))
        self.loadButton.setStatusTip(_translate("Xia2Options", "Load a pre-saved option selection"))
        ...
        self.Import_FixBeamDetector_checkBox.setText(_translate("Xia2Options", "Fix instrument model"))
        self.Import_RunSelector_checkBox.setText(_translate("Xia2Options", "Run Select"))
        ...
        self.Import_RunSelector_checkBox_13.setText(_translate("Xia2Options", "13"))
        self.Import_RunSelector_checkBox_14.setText(_translate("Xia2Options", "14"))
        self.Import_RunSelector_checkBox_15.setText(_translate("Xia2Options", "15"))
        self.Import_RunSelector_checkBox_16.setText(_translate("Xia2Options", "16"))
        self.Import_RunSelector_checkBox_17.setText(_translate("Xia2Options", "17"))
        self.Import_RunSelector_checkBox_18.setText(_translate("Xia2Options", "18"))
        self.Import_RunSelector_checkBox_19.setText(_translate("Xia2Options", "19"))
        self.Import_RunSelector_checkBox_20.setText(_translate("Xia2Options", "20"))
        self.Import_RunSelector_checkBox_21.setText(_translate("Xia2Options", "21"))
        self.Import_RunSelector_checkBox_22.setText(_translate("Xia2Options", "22"))
        self.Import_RunSelector_checkBox_23.setText(_translate("Xia2Options", "23"))
        self.Import_RunSelector_checkBox_24.setText(_translate("Xia2Options", "24"))

        self.Import_RunSelector_checkBox_25.setText(_translate("Xia2Options", "25"))
        self.Import_RunSelector_checkBox_26.setText(_translate("Xia2Options", "26"))
        self.Import_RunSelector_checkBox_27.setText(_translate("Xia2Options", "27"))
        self.Import_RunSelector_checkBox_28.setText(_translate("Xia2Options", "28"))
        self.Import_RunSelector_checkBox_29.setText(_translate("Xia2Options", "29"))
        self.Import_RunSelector_checkBox_30.setText(_translate("Xia2Options", "30"))
        self.Import_RunSelector_checkBox_31.setText(_translate("Xia2Options", "31"))
        self.Import_RunSelector_checkBox_32.setText(_translate("Xia2Options", "32"))
        self.Import_RunSelector_checkBox_33.setText(_translate("Xia2Options", "33"))
        self.Import_RunSelector_checkBox_34.setText(_translate("Xia2Options", "34"))
        self.Import_RunSelector_checkBox_35.setText(_translate("Xia2Options", "35"))
        self.Import_RunSelector_checkBox_36.setText(_translate("Xia2Options", "36"))

        self.Import_type_checkBox.setText(_translate("Xia2Options", "method"))
        self.Import_type_checkBox.setStatusTip(_translate("Xia2Options", "Different indexing algorithms for indexing."))
        ...
        self.Index_scanVarying_checkBox.setText(_translate("Xia2Options", "scan varying off"))
        self.Index_scanVarying_checkBox.setStatusTip(_translate("Xia2Options", "Does not allows models to vary during a scan."))
        ...
        self.Index_multiSweepRefine_checkBox.setText(_translate("Xia2Options", "Multi sweep refine"))
        self.Index_multiSweepRefine_checkBox.setStatusTip(_translate("Xia2Options", "Refine and process each run individually."))
        ...
        self.Integrate_scanVarying_checkBox.setText(_translate("Xia2Options", "Scan Varying Off"))
        self.Integrate_scanVarying_checkBox.setStatusTip(_translate("Xia2Options", "Does not allows models to vary during a scan."))
        ...
        self.Refine_method_comboBox.setItemText(1, _translate("Xia2Options", "dials-aimless"))
        ...
        self.Other_manualInput1_checkBox.setText(_translate("Xia2Options", "Manual Input 1"))
        self.Other_manualInput1_checkBox.setStatusTip(_translate("Xia2Options", "Manual input of xia command."))
        self.Other_manualInput2_checkBox.setText(_translate("Xia2Options", "Manual Input 2"))
        self.Other_manualInput2_checkBox.setStatusTip(_translate("Xia2Options", "Manual input of xia command."))
        self.Other_manualInput3_checkBox.setText(_translate("Xia2Options", "Manual Input 3"))
        self.Other_manualInput3_checkBox.setStatusTip(_translate("Xia2Options", "Manual input of xia command."))
        self.Other_manualInput4_checkBox.setText(_translate("Xia2Options", "Manual Input 4"))
        self.Other_manualInput4_checkBox.setStatusTip(_translate("Xia2Options", "Manual input of xia command."))

        self.Other_clusterOrLocal_label.setText(_translate("Xia2Options", "Processing computer"))
        self.Other_clusterOrLocal_label.setStatusTip(_translate("Xia2Options", "Pick if the processing is local or on a cluster node."))
        self.Other_clusterOrLocal_comboBox.setItemText(0, _translate("Xia2Options", "Cluster"))
        self.Other_clusterOrLocal_comboBox.setItemText(1, _translate("Xia2Options", "Local"))
        ...
        self.HP_correction_shaddowing_checkBox.setText(_translate("Xia2Options", "High pressure correction and dynamic shadowing"))
        self.HP_correction_shaddowing_checkBox.setStatusTip(_translate("Xia2Options", "Select this option to mask part of the image shaddowed by the cell and apply correction for beam going through diamonds"))
        self.HP_scanVarying_checkBox.setText(_translate("Xia2Options", "Scan varying off"))
        self.HP_scanVarying_checkBox.setStatusTip(_translate("Xia2Options", "Does not allows models to vary during a scan."))
        self.HP_ReferenceGeometry_checkBox.setText(_translate("Xia2Options", "Reference Geometry"))
        self.HP_ReferenceGeometry_checkBox.setStatusTip(_translate("Xia2Options", "Add starting instrument model from test crystal (/dls/i19-2/data/2020/visit/dataset/DataFiles/*.expt"))

        self.HP_ReferenceGeometry_path.setText(_translate("Xia2Options", "Path/To/instrumentModdel.expt"))
        ...
        self.HP_gasket_checkBox.setStatusTip(_translate("Xia2Options", "Generate masks for powder rings from gasket material"))
        ...
        self.HP_gasket_comboBox.setStatusTip(_translate("Xia2Options", "Select gasket material"))
        self.HP_gasketUser_checkBox.setText(_translate("Xia2Options", "Gasket (user defined)"))
        self.HP_gasketUser_checkBox.setStatusTip(_translate("Xia2Options", "Generate masks for powder rings from gasket, user defined material"))
        ...
        self.HP_FixBeamDetector_checkBox.setText(_translate("Xia2Options", "Fix instrument model"))
        self.HP_FixBeamDetector_checkBox.setStatusTip(_translate("Xia2Options", "Will fix the beam and diffractometer varibles during intergration"))
        self.HP_dmin_checkBox.setStatusTip(_translate("Xia2Options", "Area above which the pixel will be classified as strong. The number of standard deviations above the mean in the local, float"))
        self.HP_dmin_checkBox.setText(_translate("Xia2Options", "d min"))
        self.HP_UN_label.setText(_translate("Xia2Options", "Unit Cell"))
        self.HP_UN_SG_checkBox.setText(_translate("Xia2Options", "Unit Cell and Space Group"))
        self.HP_UN_SG_checkBox.setStatusTip(_translate("Xia2Options", "User input of known unit cell and space group (must provide both)."))
        self.HP_SG_label.setText(_translate("Xia2Options", "Space Group"))
        self.HP_difficulty_label_2.setText(_translate("Xia2Options", "medium difficulty options"))
        self.HP_difficulty_label_3.setText(_translate("Xia2Options", "probmatic data options"))
        self.HP_difficulty_label_1.setText(_translate("Xia2Options", "startard options"))
        self.HP_runStartEnd_checkBox.setStatusTip(_translate("Xia2Options", "Set the start/end images that each run will processed (remove images with additional cell body powder rings)"))
        ...
        self.HP_anvilThickness_checkBox.setText(_translate("Xia2Options", "Anvil Thickness"))
        self.HP_anvilThickness_checkBox.setStatusTip(_translate("Xia2Options", "Set DAC anvil thickness. Default: 1.5925 mm"))
        self.HP_anvilOpeningAngle_checkBox.setText(_translate("Xia2Options", "Anvil Opening Angle"))
        self.HP_anvilOpeningAngle_checkBox.setStatusTip(_translate("Xia2Options", "Set DAC anvil thickness. Default: 40 deg"))
